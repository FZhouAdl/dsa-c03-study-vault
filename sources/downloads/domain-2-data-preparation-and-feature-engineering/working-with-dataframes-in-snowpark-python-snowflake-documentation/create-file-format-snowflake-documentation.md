...
source: https://docs.snowflake.com/sql-reference/sql/create-file-format
...
[COPY INTO <location>](/sql-reference/sql/copy-into-location) , [COPY INTO <table>](/sql-reference/sql/copy-into-table)
...
Creates a new named file format if it doesn’t already exist, or transforms an existing file format into the one defined in the statement. A CREATE OR ALTER FILE FORMAT statement follows the syntax rules of a CREATE FILE FORMAT statement and has the same limitations as an [ALTER FILE FORMAT](/sql-reference/sql/alter-file-format) statement.

Supported alterations include changes to the formatTypeOptions and COMMENT properties. You can’t alter the TYPE property.

For more information, see CREATE OR ALTER FILE FORMAT usage notes and [CREATE OR ALTER <object>](/sql-reference/sql/create-or-alter).
[code] 
...
Specifies that the file format persists only for the duration of the [session](/user-guide/session-policies) that you created it in. A temporary file format is dropped at the end of the session.
...
>   * When you unload table data to files, Snowflake outputs _only_ to NDJSON format.
> 
...
For more information about CSV, see Usage Notes in this topic. For more information about JSON and the other semi-structured file formats, see [Introduction to loading semi-structured data](/user-guide/semistructured-intro).
...
OWNERSHIP| File format| 

  *   * Required to execute a CREATE OR ALTER FILE FORMAT statement for an _existing_ file format.
...
## CREATE OR ALTER FILE FORMAT usage notes¶
  ...
  * All limitations of the [ALTER FILE FORMAT](/sql-reference/sql/alter-file-format) command apply.
...
If you must recreate a file format after it has been linked to one or more external tables, you must recreate each of the external tables (using CREATE OR REPLACE EXTERNAL TABLE) to reestablish the association. Call the [GET_DDL](/sql-reference/functions/get_ddl) function to retrieve a DDL statement to recreate each of the external tables.

  * Conflicting file format values in a SQL statement produce an error. A conflict occurs when the same option is specified multiple times with different values (for example, `...TYPE = 'CSV' ... TYPE = 'JSON'...`).
...
Alter _my_csv_format_ so that it defines the following rules for data files and unsets the comment:
