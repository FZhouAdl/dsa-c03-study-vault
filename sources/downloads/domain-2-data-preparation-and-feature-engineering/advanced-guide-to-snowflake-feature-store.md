---
title: "Advanced Guide to Snowflake Feature Store"
source: https://quickstarts.snowflake.com/guide/advanced_guide_to_snowflake_feature_store/index.html
...
[Snowflake for Developers](https://www.snowflake.com/content/snowflake-site/global/en/developers)/[Guides](https://www.snowflake.com/content/snowflake-site/global/en/developers/guides)/Advanced Guide to Snowflake Feature Store
...
[Fork Repo](https://github.com/Snowflake-Labs/sfquickstarts/tree/master/site/sfguides/src/advanced-guide-to-snowflake-feature-store)
...
> **Note** : Snowflake Feature Store uses Snowflake Enterprise Edition features and therefore requires Enterprise Edition or higher.
...
You can create multiple Feature-Stores within a Snowflake account, for example organized by environment or business unit. Features are automatically maintained using Snowflake Dynamic Tables to ensure that the feature-engineering pipeline keeps features updated to the required level of freshness. When retrieving features for training or inference, feature-store ensures that the feature values are retrieved at the required point-in-time using Snowflake AsOf joins.
  ...
  * [Snowflake Model Registry](https://docs.snowflake.com/en/developer-guide/snowpark-ml/snowpark-ml-mlops-model-registry), which provides scalable and secure model management of ML models in Snowflake, regardless of origin. Using these features, you can build and operationalize a complete ML workflow, taking advantage of Snowflake's scale and security features.
...
**Step 1:** Download and install the miniconda installer from <https://conda.io/miniconda.html>. _(OR, you may use any other Python environment with Python 3.10, for example,[virtualenv](https://virtualenv.pypa.io/en/latest/))_.
...
Here's a sample _**connection.json**_ based on the object names mentioned in **Setup Database Environment** step.
[code] 
    {
...
For each table TPCX-AI provides three parts to the data:
  ...
  * **TRAINING** (_Development_)
  * **SCORING** (_Test_)
  * **SERVING** (_Production_)
...
> **Note:** Pay particular attention to the third code cell, and make any adjustments needed for your account/environment and user. See below:

> **Note:** If you are lacking specific elevated privileges needed for some of the operations, you may encounter issues executing some of the steps. You will need to work with an Account administrator to resolve these.
...
> **Note:** If you have only just created the Tasks you will see less execution history.
...
> **Note:** You can now adjust your _**connection.json**_ file, to reflect the database, schema and warehouse that you have created in the prior Step.

> **Note:** As before , pay particular attention to the third code cell, and make any adjustments needed for your account/environment and user. See below:
