...
source: https://docs.snowflake.com/sql-reference/data-types-logical
...
BOOLEAN can have TRUE or FALSE values. BOOLEAN can also have an UNKNOWN value, which is represented by NULL. BOOLEAN columns can be used in expressions (for example, a [SELECT](/sql-reference/sql/select) list), as well as predicates (for example, a [WHERE](/sql-reference/constructs/where) clause).
...
You can explicitly convert specific [text string](/sql-reference/data-types-text#label-character-datatypes) and [numeric](/sql-reference/data-types-numeric) values to BOOLEAN values by using the [TO_BOOLEAN](/sql-reference/functions/to_boolean) or [CAST](/sql-reference/functions/cast) functions:
...
The following example casts a text value to a BOOLEAN value. The example uses the [SYSTEM$TYPEOF](/sql-reference/functions/system_typeof) function to show the type of the value after the conversion.
[code] 
    SELECT s,
           TO_BOOLEAN(s),
...
Concatenating a computed BOOLEAN expression directly returns an error, because the expression isn’t implicitly converted to a text string:
[code] 
    SELECT 'A' || (0 = 1);
