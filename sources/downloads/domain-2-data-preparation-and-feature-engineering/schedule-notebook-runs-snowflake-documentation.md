---
title: "Schedule notebook runs | Snowflake Documentation"
source: https://docs.snowflake.com/en/user-guide/ui-snowsight/notebooks-schedule
...
When you create a schedule for running your notebook, Snowsight creates a task to run your notebook on that schedule. Snowsight executes the notebook in a non-interactive mode, cell by cell from top to bottom. The task used to run the notebook is owned by the notebook owner role and uses the notebook warehouse to run. By default, task runs of the notebook auto-suspend after 10 failures. For more information about tasks, see [Introduction to tasks](/user-guide/tasks-intro).

Each notebook run starts the notebook and connects it with the notebook warehouse. As a result, the warehouse used by the notebook is resumed and remains active until 15 minutes after the scheduled task is complete.
...
Once you create schedules for your notebook, you can view and make edits to the schedules via the task list for the schema that the notebook is in.
  ...
  4. Use the vertical ellipsis [](/static/images/snowsight/snowsight-worksheet-vertical-ellipsis.png) menu on the task that runs your notebook and select an action.



You can make edits such as changing the time or frequency of the schedule, and suspending or dropping the task completely. See [Introduction to tasks](/user-guide/tasks-intro) for more details on managing tasks.
