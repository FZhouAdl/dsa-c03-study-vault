---
title: "View tasks and task graphs in Snowsight | Snowflake Documentation"
source: https://docs.snowflake.com/user-guide/ui-snowsight-tasks
...
Tasks let you schedule the execution of SQL code. A task is associated with a specific database and schema. You can use Snowsight to view and manage your tasks and task graphs. Using Snowsight, you can also view the execution history for tasks and task graphs and retry failed tasks.
...
When you edit a task in Snowsight, the task is automatically suspended, and then resumed when you finish editing the task. For more information about suspending and resuming tasks, see [Versioning of task runs](/user-guide/tasks-intro#label-versioning-of-task-runs).
...
You can also edit the root task to change parameters for the task graph. When you edit a task in Snowsight, the task is automatically suspended and resumed when you finish editing the task. For more information about suspending and resuming tasks, see [Versioning of task runs](/user-guide/tasks-intro#label-versioning-of-task-runs).
...
You can also review task history in SQL by using a table function [TASK_HISTORY](/sql-reference/functions/task_history) or a view [TASK_HISTORY view](/sql-reference/account-usage/task_history).

### Required privileges for viewing task history¶

To view task history in Snowsight, you need the same privileges required to run the [TASK_HISTORY](/sql-reference/functions/task_history) function. That is, you must use a role that includes one of the following roles or privileges on the task:
...
The role that you use must be able to query the Account Usage [TASK_HISTORY](/sql-reference/account-usage/task_history) view. You can grant the USAGE_VIEWER database role in the shared SNOWFLAKE database to the role that you use.
...
For details, see:
  ...
  * [ACCOUNT_USAGE schema SNOWFLAKE database roles](/sql-reference/account-usage#label-account-usage-snowflake-db-roles)
...
Task run history includes details about each execution of a given task. You can view the scheduled run time, status, return value, duration of a task, and other information.
...
For example, to identify long-running tasks, select the **Status** filter to show only successful tasks, and sort the **Duration** field in descending order. For advanced debugging, you can open the filtered and sorted table in worksheets using the **Open in worksheets** button. You could then modify the SQL statement with [LIMIT / FETCH](/sql-reference/constructs/limit) and [GROUP BY](/sql-reference/constructs/group-by) arguments to identify the databases and schemas with the top 25 most long-running tasks.
