...
source: https://docs.snowflake.com/en/developer-guide/snowflake-ml/feature-store/overview
...
The Snowflake Feature Store is fully integrated with the [Snowflake Model Registry](/developer-guide/snowflake-ml/model-registry/overview) and other Snowflake ML features for end-to-end production ML.
...
A feature store contains [feature views](/developer-guide/snowflake-ml/feature-store/feature-views). A feature view encapsulates a Python or SQL pipeline for transforming raw data into one or more related features. All features defined in a feature view are refreshed from the source data at the same time.
...
All objects of a Snowflake Feature Store are stored in the feature store’s schema. To completely delete a feature store, make sure the schema doesn’t contain any other resources, and then [drop the schema](/sql-reference/sql/drop-schema).

## Getting started¶

Note

The Snowflake Feature Store Python API is part of the Snowpark ML Python package, `snowflake-ml-python`. You can use it on your local system in your preferred Python IDE or in a Snowsight worksheet or notebook. For details, see [Python APIs for Snowflake ML](/developer-guide/snowflake-ml/snowpark-ml).
...
See [Example feature patterns](/developer-guide/snowflake-ml/feature-store/examples) for SQL examples of common transformation shapes, and [Advanced feature engineering](/developer-guide/snowflake-ml/feature-store/advanced-feature-engineering) for time-windowed aggregation and other production patterns.
