...
source: https://docs.snowflake.com/en/user-guide/snowsql-config
...
You can change the default location by specifying the `--config _path_` command-line flag when starting SnowSQL.
...
[/code]
  ...
  * If a value contains quote characters _in addition to_ other special characters, escape these quotes using the backslash (`\`) character, e.g.:
[code] password = "my$^\"\'pwd"
...
[code] prompt_format='''[#FFFF00][user]@[account]
        [#00FF00]> '''
...
>     
> 
> Removes trailing semicolons from SQL text before sending queries to Snowflake. Note that removing the semicolons can prevent Snowflake from using cached results from different clients when the [USE_CACHED_RESULT](/sql-reference/parameters#label-use-cached-result) session parameter is enabled.
> 
> Default:
>     
> 
