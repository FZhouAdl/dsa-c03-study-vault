---
title: "TensorFlow | Snowflake Documentation"
source: https://docs.snowflake.com/developer-guide/snowflake-ml/model-registry/built-in-models/tensorflow
...
The Snowflake ML Model Registry supports models created using TensorFlow (models derived from `tensorflow.Module`) and Keras v2 models (`keras.Model` with Keras version < 3.0.0).
...
When using pandas DataFrames (which use float64 by default), ensure your TensorFlow model uses `tf.float64` for variables and `tf.TensorSpec` input signatures to avoid dtype mismatch errors.
