---
title: "About Secure Data Sharing | Snowflake Documentation"
source: https://docs.snowflake.com/user-guide/data-sharing-intro
...
Snowflake enables the sharing of databases through _shares_ , which are created by data providers and “imported” by data consumers.
...
You can create as many shares as you want, and add as many accounts to a share as you want. If you want to provide a share to many accounts, you might want to use a [listing](/collaboration/collaboration-listings-about) or a [data exchange](/user-guide/data-exchange).

For a guide to sharing data as a provider, refer to [Share secure database objects](/user-guide/data-sharing-gs). For more detailed information, refer to [Create and configure shares](/user-guide/data-sharing-provider).
...
For details about usage data for listings, refer to [Monitor listing use](/collaboration/provider-listings-monitor-studio). Usage data for listings shared in a data exchange is only available in the views contained in the [Data Sharing Usage](/sql-reference/data-sharing-usage) schema of the imported Snowflake database.
...
Each reader account belongs to the provider account that created it. As a provider, you use _shares_ to share databases with reader accounts; however, a reader account can only consume data from the provider account that created it. Refer to the following diagram:
