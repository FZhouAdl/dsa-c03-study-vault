...
source: https://docs.snowflake.com/en/developer-guide/snowflake-ml/model-registry/model-explainability
...
Explainability is available by default for the above models logged using Snowpark ML 1.6.2 and later. The implementation uses the [SHAP library](https://pypi.org/project/shap/).
...
Models that were logged in the registry using a version of Snowpark ML older than 1.6.2 do not have the explainability feature. Since model versions are immutable, you must create a new model version to add explainability to an existing model. You can use `ModelVersion.load` to retrieve the Python object representing the model’s implementation, then log that to the registry as a new model version. Be sure to pass your background data as `sample_input_data`. This approach is shown below.

Important

The Python environment into which you load the model must be exactly the same (that is, the same version of Python and of all libraries) as the environment where the model is deployed. For details, see [Loading a model version](/developer-guide/snowflake-ml/model-registry/overview#label-snowpark-model-registry-model-versions-loading).
[code] 
