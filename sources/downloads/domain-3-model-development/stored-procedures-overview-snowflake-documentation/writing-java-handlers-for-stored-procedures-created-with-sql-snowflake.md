---
title: "Writing Java handlers for stored procedures created with SQL | Snowflake Documentation"
source: https://docs.snowflake.com/developer-guide/stored-procedure/java/procedure-java-overview
...
You can create a stored procedure whose handler is written in Java. You can use the [Snowpark library](/developer-guide/snowpark/java/setup) within your stored procedure to perform queries, updates, and other work on tables in Snowflake.

With stored procedures, you can build and run your data pipeline within Snowflake, using a Snowflake warehouse as the compute framework. For the code for your data pipeline, you use the Snowpark API for Java to write stored procedures. To schedule the execution of these stored procedures, you use [tasks](/user-guide/tasks-intro).
...
## Writing a Java handler for a stored procedure¶
  ...
  4. Follow guidelines for the handler class, method, and performance.
  5. Implement support for features such as [data access](/developer-guide/stored-procedure/java/procedure-java-access-data), [file reading](/developer-guide/stored-procedure/java/procedure-java-read-files), [returning tabular data](/developer-guide/stored-procedure/java/procedure-java-tabular-data), and [logging and tracing](/developer-guide/logging-tracing/logging-java).
...
If you’re developing your code locally, set up your development environment to use the Snowpark library. See [Setting Up Your Development Environment for Snowpark Java](/developer-guide/snowpark/java/setup).

## Structuring and building handler code¶

You can keep handler source code in-line with the SQL that creates the procedure or keep handler compiled result in a separate location and reference it from the SQL. For more information, see [Keeping handler code in-line or on a stage](/developer-guide/inline-or-staged).
...
When you call your stored procedure, Snowflake automatically creates a `Session` object and passes it to your stored procedure. (You cannot create the `Session` object yourself.)
  ...
  * For the rest of the arguments and for the return value, use the [Java types](/developer-guide/udf-stored-procedure-data-type-mapping#label-sql-java-data-type-mappings) that correspond to [Snowflake data types](/sql-reference-data-types).
  ...
  * When you run an [asynchronous child job](/developer-guide/snowpark/java/working-with-dataframes#label-snowpark-java-dataframe-action-method-async) from within a procedure’s handler, “fire and forget” is not supported.
...
If an uncaught exception occurs inside the method, Snowflake raises an error that includes the stack trace for the exception. When [logging of unhandled exceptions](/developer-guide/logging-tracing/unhandled-exception-messages) is enabled, Snowflake logs data about unhandled exceptions in an event table.
