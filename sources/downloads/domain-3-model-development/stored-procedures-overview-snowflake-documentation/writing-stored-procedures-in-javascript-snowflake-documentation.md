...
source: https://docs.snowflake.com/developer-guide/stored-procedure/stored-procedures-javascript
...
These objects are described in detail in the [JavaScript stored procedures API](/developer-guide/stored-procedure/stored-procedures-api).
...
For more information about how Snowflake maps JavaScript and SQL data types, see [SQL-JavaScript Data Type Mappings](/developer-guide/udf-stored-procedure-data-type-mapping#label-javascript-supported-snowpark-types).
...
[code] var sql_command = "SELECT col1, col2"
...
You can execute your SQL statements inside a try block. If an error occurs, then your catch block can roll back all of the statements (if you put the statements in a transaction). The Examples section contains an example of rolling back a transaction in a stored procedure.
...
For information about overloading and naming conventions, see [Naming and overloading procedures and UDFs](/developer-guide/udf-stored-procedure-naming-conventions).

### Binding variables¶

[Binding](/sql-reference/bind-variables) a variable to a SQL statement allows you to use the value of the variable in the statement.

You can bind NULL values as well as non-NULL values.

The data type of the variable should be appropriate for the use of the value in the SQL statement. Currently, only JavaScript variables of type number, string, and [SfDate](/developer-guide/stored-procedure/stored-procedures-api#label-sfdate) can be bound. (For details about the mapping between SQL data types and JavaScript data types, see SQL and JavaScript data type mapping.)
...
This shows how to bind a VARCHAR, a TIMESTAMP_LTZ, and other data types to an `INSERT` statement. The TIMESTAMP_LTZ binds an [SfDate](/developer-guide/stored-procedure/stored-procedures-api#label-sfdate) variable that is created inside the stored procedure.
...
[/code]
> 
> Create a stored procedure. This procedure accepts a `VARCHAR`, and converts the VARCHAR to a `TIMESTAMP_LTZ` by using SQL. The procedure then retrieves the converted value from a ResultSet. The value is stored in a JavaScript variable of type [SfDate](/developer-guide/stored-procedure/stored-procedures-api#label-sfdate). The stored procedure then binds both the original `VARCHAR` and the `TIMESTAMP_LTZ` to an `INSERT` statement. This also demonstrates binding of JavaScript numeric data.
[code]
...
For additional examples of binding data in JavaScript, see [Binding statement parameters](/developer-guide/node-js/nodejs-driver-execute#label-nodejs-binding).
...
Snowflake limits the maximum size of the JavaScript source code in the body of a JavaScript stored procedure. Snowflake recommends limiting the size to 100 KB. (The code is stored in a compressed form, and the exact limit depends on the compressibility of the code.)
...
> 
[code]CREATE TABLE stproc_test_table1 (num_col1 numeric(14,7));
>          
[/code]
...
> 
[code]select * from stproc_test_table1;
>          +-----------+
>          |  NUM_COL1 |
>          |-----------|
>          | 5.1400000 |
...
The following example retrieves a result:
  ...
  1. Create a procedure to count the number of rows in a table (equivalent to `select count(*) from _table_`):

> 
[code]CREATE OR REPLACE PROCEDURE get_row_count(table_name VARCHAR)
>            RETURNS FLOAT NOT NULL
>            LANGUAGE JAVASCRIPT
>            AS
>            $$
...
> 
[code]call get_row_count('stproc_test_table1');
>          +---------------+
>          | GET_ROW_COUNT |
>          |---------------|
>          |             3 |
...
> 
[code]select count(*) from stproc_test_table1;
>          +----------+
>          | COUNT(*) |
>          |----------|
>          |        3 |
...
As stated in [SQL injection](/developer-guide/stored-procedure/stored-procedures-usage#label-sql-injection) (in this topic), be careful to guard against attacks when using dynamic SQL.

  1. Create the stored procedure. This procedure allows you to pass the name of a table and get the number of rows in that table (equivalent to `select count(*) from _table_name_`):
...
>   1. Create the stored procedure:
[code] CREATE OR REPLACE PROCEDURE validate_age (age float)
>          RETURNS VARCHAR
>          LANGUAGE JAVASCRIPT
>          EXECUTE AS CALLER
>          AS $$
...
The following example wraps multiple related statements in a transaction, and uses try/catch to commit or roll back. The parameter `force_failure` allows the caller to choose between successful execution and deliberate error.
[code] 
...
You can capture log and trace data from JavaScript handler code by using the `snowflake` object in the JavaScript API. When you do, log messages and trace data are stored in an event table that you can analyze with queries.

For more information, refer to the following:

  * [Logging messages in JavaScript](/developer-guide/logging-tracing/logging-javascript)
  * [Emitting trace events in JavaScript](/developer-guide/logging-tracing/tracing-javascript)



### Using RESULT_SCAN to retrieve the result from a stored procedure¶

The following example shows you how to use the [RESULT_SCAN](/sql-reference/functions/result_scan) function to retrieve and process the result from a [CALL](/sql-reference/sql/call) statement:

  1. Create and load the table:

> 
[code]CREATE TABLE western_provinces(ID INT, province VARCHAR);
>          
[/code]
[code]INSERT INTO western_provinces(ID, province) VALUES
>           (1, 'Alberta'),
>           (2, 'British Columbia'),
>           (3, 'Manitoba')
>           ;
...
> 
[code]CREATE OR REPLACE PROCEDURE read_western_provinces()
>            RETURNS VARCHAR NOT NULL
>            LANGUAGE JAVASCRIPT
>            AS
>            $$
...
> 
[code]CALL read_western_provinces();
>          +------------------------+
...
You can also use the [pipe operator](/sql-reference/operators-flow) (`->>`) instead of the RESULT_SCAN function to run a CALL statement and process its result set with a single command.
...
> 
[code]CREATE TABLE all_provinces(ID INT, province VARCHAR);
>          
[/code]
...
> 
[code]INSERT INTO all_provinces
>            WITH
...
> 
[code]CREATE TRANSIENT TABLE one_string(string_col VARCHAR);
>          
[/code]
...
> 
[code]SELECT string_col FROM one_string;
>          +---------------------+
>          | STRING_COL          |
>          |---------------------|
>          | 1, Alberta          |
...
> 
[code]SELECT * FROM one_string, LATERAL SPLIT_TO_TABLE(one_string.string_col, '\n');
>          +---------------------+-----+-------+---------------------+
...
> 
[code]CREATE TRANSIENT TABLE three_strings(string_col VARCHAR);
>          
[/code]
...
> 
[code]INSERT INTO three_strings
>            SELECT VALUE FROM one_string, LATERAL SPLIT_TO_TABLE(one_string.string_col, '\n');
>          +-------------------------+
>          | number of rows inserted |
>          |-------------------------|
...
> 
[code]create table reviews (customer_ID VARCHAR, review VARCHAR);
>          create table purchase_history (customer_ID VARCHAR, price FLOAT, paid FLOAT,
>                                      product_ID VARCHAR, purchase_date DATE);
>          
[/code]
[code]insert into purchase_history (customer_ID, price, paid, product_ID, purchase_date) values
...
> 
[code]create or replace procedure delete_nonessential_customer_data(customer_ID varchar)
>           returns varchar not null
>           language javascript
>           as
>           $$
...
> 
[code]call delete_nonessential_customer_data(1);
>          +---------------------------------------------------------+
...
> 
[code]call delete_nonessential_customer_data(2);
>          +---------------------------------------------------------+
>          | DELETE_NONESSENTIAL_CUSTOMER_DATA                       |
>          |---------------------------------------------------------|
...
> 
[code]call delete_nonessential_customer_data(3);
>          +-------------------------------------------------------------------------+
>          | DELETE_NONESSENTIAL_CUSTOMER_DATA                                       |
>          |-------------------------------------------------------------------------|
...
> 
[code]create table sv_table (f float);
...
> 
[code]create procedure session_var_user()
>            returns float
>            language javascript
>            EXECUTE AS CALLER
>            as
...
> 
[code]create procedure cannot_use_session_vars()
>            returns float
>            language javascript
>            EXECUTE AS OWNER
>            as
...
> 
[code]CALL cannot_use_session_vars();
>          
[/code]
...
> 
[code]create procedure cannot_set_session_vars()
>            returns float
>            language javascript
>            EXECUTE AS OWNER
>            as
...
> 
[code]CALL cannot_set_session_vars();
>          
[/code]
...
The following sections provide additional suggestions to help debug specific problems.
...
For more details, see [JavaScript arguments and returned values](/developer-guide/udf/javascript/udf-javascript-introduction#label-js-udf-arguments).
...
Your stored procedure might contain “sqltext” when it should have “sqlText” (the first is all lowercase; the second is mixed case).
