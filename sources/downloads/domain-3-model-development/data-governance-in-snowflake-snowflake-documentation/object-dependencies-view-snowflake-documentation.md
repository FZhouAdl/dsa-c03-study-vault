---
title: "OBJECT_DEPENDENCIES view | Snowflake Documentation"
source: https://docs.snowflake.com/sql-reference/account-usage/object_dependencies
...
Note that if a view or [UDF](/developer-guide/udf/udf-overview) was invalid due to a missing dependency prior to this date and the missing dependency is fixed later, Snowflake does not record the dependency for the view or UDF.
...
For example, the shared database contains a view named `db1_shared.views.view_1_shared`. The consumer renames the shared database to `mydb`. The view now has a fully-qualified name of `mydb.views.view_1_shared`. Snowflake removes the row specifying `db1_shared.views.view_1_shared` in the consumer’s OBJECT_DEPENDENCIES view because the dependency on the database named `db1_shared` is broken.
