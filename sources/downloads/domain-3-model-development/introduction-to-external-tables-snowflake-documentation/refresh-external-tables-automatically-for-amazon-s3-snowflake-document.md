...
source: https://docs.snowflake.com/user-guide/tables-external-s3
...
Note
  ...
  * Note that currently, accessing S3 storage in [government regions](/user-guide/intro-regions#label-us-gov-regions) using a storage integration is limited to Snowflake accounts hosted on AWS in the same government region. Accessing your S3 storage from an account hosted outside of the government region using direct credentials is supported.
...
Complete the following steps to configure access permissions for Snowflake to access your S3 bucket:
  ...
  4. Under **Security Token Service (STS)** in the **Endpoints** list, find the Snowflake [region](/user-guide/intro-regions) where your account is located.
...
Note
  ...
  10. Replace `_bucket_` and `_prefix_` with your actual bucket name and folder path prefix.
...
Optionally, use the [STORAGE_AWS_EXTERNAL_ID](/sql-reference/sql/create-storage-integration#label-create-storage-integration-aws-ext-id) parameter to specify your own external ID. You might choose this option to use the same external ID across multiple external volumes and/or storage integrations.
...
[code] DESC INTEGRATION <integration_name>;
...
> Note
> 
> For security reasons, if you create a new storage integration (or recreate an existing storage integration using the CREATE OR REPLACE STORAGE INTEGRATION syntax) without specifying an external ID, the new integration has a _different_ external ID and can’t resolve the trust relationship unless you update the trust policy.
...
Before you proceed, determine whether an S3 event notification exists for the target path (or _prefix_ , in AWS terminology) in your S3 bucket where your data files are located. AWS rules prohibit creating conflicting notifications for the same path.
...
## Step 1: Subscribe the Snowflake SQS Queue to the SNS Topic¶
  ...
  5. Using a Snowflake client, query the [SYSTEM$GET_AWS_SNS_IAM_POLICY](/sql-reference/functions/system_get_aws_sns_iam_policy) system function with your SNS topic ARN:

> 
[code]select system$get_aws_sns_iam_policy('<sns_topic_arn>');
>          
[/code]
...
> 
[code]select system$get_aws_sns_iam_policy('arn:aws:sns:us-west-2:001234567890:s3_mybucket');
>          
>          +---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Create an external table by using [CREATE EXTERNAL TABLE](/sql-reference/sql/create-external-table). Identify the SNS topic ARN from Prerequisite: Create an Amazon SNS Topic and Subscription:
[code] 
