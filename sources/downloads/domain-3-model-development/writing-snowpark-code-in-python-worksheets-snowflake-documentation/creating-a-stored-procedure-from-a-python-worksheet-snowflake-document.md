---
title: "Creating a stored procedure from a Python worksheet | Snowflake Documentation"
source: https://docs.snowflake.com/developer-guide/stored-procedure/python/procedure-python-create-worksheet
...
For example, you might write code in a Python worksheet that extracts data from stages or database objects in Snowflake, transforms the data, then stores the transformed data in Snowflake. You could then deploy that code as a stored procedure and build a data pipeline, all without leaving Snowflake.

Create a Python stored procedure from your Python worksheet to automate your code. For details on writing Python worksheets, see [Writing Snowpark Code in Python Worksheets](/developer-guide/snowpark/python/python-worksheets#label-snowsight-py-worksheets-write).
...
To create a Python stored procedure that automates the code in your Python worksheet, do the following:
  ...
  9. Review the arguments used by your handler function and, if needed, override the SQL data type mapping for a typed argument. For details about how Python types are mapped to SQL types, see [SQL-Python Data Type Mappings](/developer-guide/udf-stored-procedure-data-type-mapping#label-sql-python-data-type-mappings).
