...
source: https://docs.snowflake.com/user-guide/tables-temp-transient
...
Snowflake supports creating temporary tables for storing non-permanent, transitory data (for example, ETL or session-specific data). By default, a temporary table created with [CREATE TEMPORARY TABLE](/sql-reference/sql/create-table) exists only in the [session](/user-guide/session-policies) in which it was created and persists for the remainder of that session. As such, the table is not visible to other users or sessions. When the session ends, data in the table is purged and is not recoverable, either for the user who created the table or for Snowflake.

### Procedure-scoped temporary tables¶

In addition to session-scoped temporary tables, you can create a procedure-scoped temporary table from inside a [Snowflake Scripting](/developer-guide/snowflake-scripting/index) stored procedure by running `CREATE OR REPLACE PROCEDURE SCOPED TEMP TABLE` in the procedure body. That table exists only for the duration of a single execution of the procedure, not for the rest of the session. For syntax and details, see [Procedure-scoped temporary tables (Snowflake Scripting)](/sql-reference/sql/create-procedure#label-create-procedure-scoped-temp-table) in the [CREATE PROCEDURE](/sql-reference/sql/create-procedure) reference. For a short example, see [procedure-scoped temporary table example](/developer-guide/stored-procedure/stored-procedures-snowflake-scripting#label-stored-procedures-snowscript-scoped-temp-table) in [Writing stored procedures in Snowflake Scripting](/developer-guide/stored-procedure/stored-procedures-snowflake-scripting).
...
If your workload generates high volumes of temporary tables, you are very likely to experience degraded performance when you query the [COLUMNS view](/sql-reference/info-schema/columns) or [TABLES view](/sql-reference/info-schema/tables) in the Information Schema.
...
To create a temporary table, simply specify the TEMPORARY keyword (or TEMP abbreviation) in [CREATE TABLE](/sql-reference/sql/create-table). You can also use the [TableCollection.create](/developer-guide/snowflake-python-api/reference/latest/_autosummary/snowflake.core.table.TableCollection#snowflake.core.table.TableCollection.create) Python API.
...
When you create a transient table as a clone of a permanent table, Snowflake creates a [zero-copy clone](/user-guide/tables-storage-considerations#label-cloning-tables). This means when the transient table is created, it utilizes no data storage because it shares all of the existing [micro-partitions](/user-guide/tables-clustering-micropartitions) of the original permanent table. When rows are added, deleted, or updated in the clone, it results in new micro-partitions that belong exclusively to the clone (in this case, the transient table).
...
---|---|---|---|---  
Temporary| Remainder of session| Temporary => Temporary   
  
Temporary => Transient| 0 or 1 (default is 1)| 0  
Transient| Until explicitly dropped| Transient => Temporary   
  
Transient => Transient| 0 or 1 (default is 1)| 0  
Permanent ([Standard Edition](/user-guide/intro-editions))| Until explicitly dropped| Permanent => Temporary   
  
Permanent => Transient   
  
Permanent => Permanent| 0 or 1 (default is 1)| 7  
Permanent ([Enterprise Edition and higher](/user-guide/intro-editions))| Until explicitly dropped| Permanent => Temporary   
  
Permanent => Transient   
  
Permanent => Permanent| 0 to 90 (default is configurable)| 7  
