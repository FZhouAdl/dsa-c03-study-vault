...
source: https://docs.snowflake.com/user-guide/views-materialized
...
Materialized views are particularly useful when:
  ...
  * The query is on an [external table](/user-guide/tables-external-intro), which might have slower performance compared to querying native database tables or [Apache Iceberg™ tables](/user-guide/tables-iceberg).
...
For more information about secure views, see [Working with Secure Views](/user-guide/views-secure).

The syntax to create secure materialized views is documented at [CREATE MATERIALIZED VIEW](/sql-reference/sql/create-materialized-view).
...
Use the [CREATE MATERIALIZED VIEW](/sql-reference/sql/create-materialized-view) command to create a materialized view. For an example, see Basic Example: Creating a Materialized View (in this topic).
...
[code] -- Example of a materialized view with a range filter
        create materialized view v1 as
          select * from table1 where column_1 between 100 and 400;
        
[/code]
[code] -- Example of a query that might be rewritten to use the materialized view
...
[code] create materialized view mv1 as
...
[code] select * from tab1 where column_1 = Y;
        
[/code]

The query above can be rewritten internally as:
[code] select * from mv1 where column_1 = Y;
...
[code] create materialized view mv2 as
          select * from table1 where column_1 = X;
        
[/code]

Create a query:
[code] select column_1, column_2 from table1 where column_1 = X AND column_2 = Y;
        
[/code]

The query can be rewritten as:
[code] select * from mv2 where column_2 = Y;
...
[code] create materialized view mv4 as
...
[code] select column_1, sum(column_3) from table1 group by column_1;
        
[/code]

The query can be rewritten as:
[code] select column_1, sum(column_3) from mv4 group by column_1;
...
[code] OVER ( [ PARTITION BY <expr1> ] [ ORDER BY <expr2> ] )
...
[code] create materialized view bad_example (ts1) as
...
[code] create materialized view good_example (ts1) as
...
The values in the SEQ column are not guaranteed to be ordered in any way when selected from a materialized view. If you select this column in the materialized view definition, the output may be indeterministic.
  ...
  * Materialized views cannot be created using the [Time Travel feature](/user-guide/data-time-travel).
...
[code] Failure during expansion of view 'MY_MV':
          SQL compilation error: Materialized View MY_MV is invalid.
          Invalidation reason: Division by zero
        
[/code]

If this occurs, address the problem described in the error message (for example, delete the rows that introduce the “divide by zero” error), and resume the materialized view by using the [ALTER MATERIALIZED VIEW … RESUME](/sql-reference/sql/alter-materialized-view) command.
...
If you need to suspend the maintenance and use of a materialized view, execute the [ALTER MATERIALIZED VIEW](/sql-reference/sql/alter-materialized-view) command with the SUSPEND parameter:
[code] 
...
The [INFORMATION_SCHEMA.VIEWS view](/sql-reference/info-schema/views) does not show materialized views. Materialized views are shown by INFORMATION_SCHEMA.TABLES.
...
Truncating a materialized view is not supported.
  ...
  * Snowflake does not support using the [Time Travel feature](/user-guide/data-time-travel) to query materialized views at [a point in the past](/user-guide/data-time-travel#label-time-travel-querying) (e.g. using the [AT clause](/sql-reference/constructs/at-before) when querying a materialized view).

However, you can use Time Travel to [clone a database or schema containing a materialized view at a point in the past](/user-guide/data-time-travel#label-time-travel-cloning). For details, see Materialized Views and Time Travel.

  * Materialized Views are not monitored by Snowflake [Working with resource monitors](/user-guide/resource-monitors).
...
You can view the billing costs for maintaining materialized views using [Snowsight](/user-guide/ui-snowsight-gs#label-snowsight-getting-started-sign-in) or SQL:

Snowsight:
    

As a user with the [proper privileges](/user-guide/cost-access-control), In the navigation menu, select **Admin** » **Cost management** , and then select **Consumption**.

Use the **All Service Types** filter and select **Materialized View**.

The credit costs are tracked in a Snowflake-provided virtual warehouse named [](/static/images/logo-snowflake-sans-text.png) **MATERIALIZED_VIEW_MAINTENANCE**.
...
For example:
[code] SELECT * FROM TABLE(INFORMATION_SCHEMA.MATERIALIZED_VIEW_REFRESH_HISTORY());
...
[code] SELECT TO_DATE(start_time) AS date,
          database_name,
          schema_name,
          table_name,
...
[Resource monitors](/user-guide/resource-monitors) provide control over virtual warehouse credit usage; however, you cannot use them to control credit usage for the Snowflake-provided warehouses, including the [](/static/images/logo-snowflake-sans-text.png) **MATERIALIZED_VIEW_MAINTENANCE** warehouse.
...
Currently, you cannot use [Time Travel](/user-guide/data-time-travel) to [query historical data](/user-guide/data-time-travel#label-time-travel-querying) for materialized views.

However, note the following:

  * You can use Time Travel to [clone a database or schema containing a materialized view at a specific point in the past](/user-guide/data-time-travel#label-time-travel-cloning). Snowflake clones the materialized view at the specified point in time.
  * To support cloning with Time Travel, Snowflake does maintain historical data for materialized views. You will be billed for the [storage costs for historical data](/user-guide/data-cdp-storage-costs) for materialized views.
  * The storage costs depend on the [data retention period](/user-guide/data-time-travel#label-time-travel-data-retention-period) for the materialized views, which is determined by the [DATA_RETENTION_TIME_IN_DAYS](/sql-reference/parameters#label-data-retention-time-in-days) parameter. Materialized views inherit this parameter from its parent schema or database.
...
If you create a materialized view on a clustered table, consider removing any clustering on the base table, because any change to the clustering of the base table will eventually require a refresh of the materialized view, which adds to the materialized view’s maintenance costs.
  ...
  * Almost all information about clustering tables also applies to clustering materialized views. For more information about clustering tables, see [Strategies for Selecting Clustering Keys](/user-guide/tables-clustering-keys#label-clustering-keys-strategies).
...
If this occurs, address the problem described in the error message, and resume the materialized view by using the [ALTER MATERIALIZED VIEW … RESUME](/sql-reference/sql/alter-materialized-view) command.
...
One possible cause is that the refresh failed because the [SELECT](/sql-reference/sql/select) statement in the view definition failed.

Because the refresh is done performed by the background process, you will not see an error message at the time the refresh is attempted. Instead, you will see the error message when you query the materialized view or when you execute [SHOW MATERIALIZED VIEWS](/sql-reference/sql/show-materialized-views).
...
If you do not know the exact definition of the materialized view, you can find it in the output of [SHOW MATERIALIZED VIEWS](/sql-reference/sql/show-materialized-views) or by using the [GET_DDL](/sql-reference/functions/get_ddl) function.
